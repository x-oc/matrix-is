...
public interface UnitRepository extends JpaRepository<Unit, Long> {
    List<Unit> findByStatus(UnitStatusEnum status);
    ...
    List<Unit> findCandidates(@Param("threshold") Double threshold,
                              @Param("statuses") List<UnitStatusEnum> statuses);

    @Query("SELECT u FROM Unit u WHERE u.disagreementIndex > 8.5 AND u.status = 'NORMAL'")
    List<Unit> findNewCandidates();
}